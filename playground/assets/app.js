(() => {
  ...
  const ICONS = {
    ...
    warning: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3" /><path d="M12 9v4" /><path d="M12 17h.01" /></svg>',
    ...
    // AST node icons — one per node type
  ...
  function renderFormatTabs() {
    ...
    formatConfig.forEach((format) => {
      ...
      button.type = "button";
      button.className = `format-pill${format.value === selectedFormat ? " is-active" : ""}`;
      button.innerHTML = `
        <span class="icon-shell">${icon(format.icon)}</span>
        ${format.label}
      `;
      ...
      formatTabs.append(button);
    });
  }

  function renderExampleList() {
    const items = formatExamples(selectedFormat);
    exampleCount.textContent = `${items.length} scenarios`;
  ...
  function renderDiagnosticsFooter(result) {
    ...
    if (unknownTags.length > 0) {
      ...
      tagsHtml = `
        <div class="footer-tags">
          <span class="footer-section-label">Unknown tags</span>
          ${tagChips}
        </div>
      `;
    }
    ...
    if (diagParts.length > 0) {
      diagHtml = `
        <div class="footer-diagnostics">
          <span class="footer-section-label">Diagnostics</span>
          ${diagParts.map((part) => `<span class="footer-diagnostic">${escapeHtml(part)}</span>`).join("")}
        </div>
      `;
    }
    ...
    diagnosticsFooter.innerHTML = `
      <div class="footer-stats">${statsHtml}</div>
      ${tagsHtml}
      ${diagHtml}
    `;
  }
  ...
  function nodeIcon(node) {
    return NODE_ICONS[node.type] || "generic";
  }
