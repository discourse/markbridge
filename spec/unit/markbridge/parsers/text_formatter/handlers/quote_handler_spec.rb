...
RSpec.describe Markbridge::Parsers::TextFormatter::Handlers::QuoteHandler do
  ...
  describe "#process" do
    ...
    it "parses leading-zero ids as decimal instead of octal" do
      handler.process(element: build_element('<QUOTE post_id="099" user_id="010"/>'), parent:)

      quote = parent.children[0]
      expect(quote.post_id).to eq(99)
      expect(quote.user_id).to eq(10)
    end
