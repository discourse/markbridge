...
- Branch: `main`
- Bench scripts: [`bench/bench.rb`](../bench/bench.rb) for the short
  reports, [`bench/sustained_bench.rb`](../bench/sustained_bench.rb)
...
- `ruby 4.0.5` with YJIT (+PRISM) — **project default**
...
- **Ruby 4.0 is faster than 3.3 and 3.4 on every conversion path**, by
  17% to 31%. The two escape reports are the exception: they are flat
  (`escape_plain`) or slower (`escape_mixed`, see below). 4.0 stays the
  right default for CI and development.
- **TruffleRuby 40 is 2.2x to 4.2x faster than Ruby 4.0 on the
  conversion paths.** This is not an artifact of the benchmark: with a
  rotating set of inputs and the rendered Markdown consumed
  (`.markdown.bytesize` summed into a counter), TruffleRuby keeps the
  same 4x on the `simple` path, so Graal is not skipping the work. Two
  things to keep in mind before reading this as a 4x for real
  workloads. The reports run one small input in a loop, which is what
  Graal optimizes best. And TruffleRuby needs seconds to reach this
  speed, so a short-lived script that converts a few hundred posts and
  exits will see much less. The sustained corpus run below measures
  both effects.
- **Ruby 3.4 and 4.0 have a YJIT regression on `escape_mixed`** vs 3.3
  (7.5k and 7.8k vs 11.4k, −34% and −32%). 3.3's YJIT handles the
  inline-byte dispatch loop better. Worth pinging the YJIT team if it
  matters.
- **JRuby is slower than Ruby 4.0 on every conversion path** in this
  run, by 17% to 37%, with `table` and `list` the worst. It wins on the
  escape reports: `escape_mixed` **+111%**, `escape_plain` **+30%** and
  `escaping` **+10%**. Its JIT does well on the inline-byte loops and
  badly on the allocation-heavy render paths.
- **`escape_plain` is the flattest report** across the CRuby versions
  (~396–403k i/s). The `MAYBE_SPECIAL.match?` check returns the
  original string with no allocations, so there is little left for the
  JIT to optimize — the JVM engines still find something.
...
[`bench/corpus.rb`](../bench/corpus.rb)) converted in a loop for 60
